...
const renderComponent = (contextValue: StockItemsContextProps) => {
    return render(
        <StockItemsContext.Provider value={contextValue}>
            <StockItems ID="1"/>
...
describe('StockItems', () => {
    ...
    it('appelle fetchStockItems lors du montage', async () => {
        const mockSetStockItems = vi.fn();
        const fetchStockItemsMock = vi.fn().mockResolvedValue([{ID: 1, LABEL: 'Item 1', QUANTITY: 5}]);
        ...
        await waitFor(() => expect(fetchStockItemsMock).toHaveBeenCalledWith(1));  // 1 est l'ID du stock
    });
